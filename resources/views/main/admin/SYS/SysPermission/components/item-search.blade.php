<form action="" action="" id="requestSearch" method="GET">
    ...
    <div style="" class="row">
            ...
            <div class="form-group">
                <div class="kt-input-icon kt-input-icon--right">
                    <input type="text" name="name_alias" class="form-control"
                           ...
                           id="name_alias" value="{{$filter['name_alias']??''}}">
        ...
        <div class="col-md-3">
            <div class="form-group">
                <select class="form-control select2 kt-select2" id="list_module" name="module_id">
                    <option value="" selected>Chọn module</option>
                    @foreach ($listModule as $key => $items )
                        <option
                            {{ isset($filter['module_id']) &&  $filter['module_id'] == $items['id'] ? 'selected' : ''}} value="{{ $items['id']}}"> {{$items['module_alias']}} </option>
                    @endforeach
                </select>
            </div>
        </div>
