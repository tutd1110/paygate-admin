                    ...
                    <div class="kt-grid kt-wizard-v3--white" id="kt_wizard_v3" data-ktwizard-state="step-first">
                        <div class="kt-grid__item kt-grid__item--fluid kt-wizard-v3 kt-wizard-v3__wrapper">
                            <form class="px-5 py-3 kt-form kt-form--label-right" id="addOrders"
                                  action="{{route('pgw_orders.store')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="kt-wizard-v3__content" data-ktwizard-type="step-content"
                                     data-ktwizard-state="current">
                                    @include('main.admin.order.components.item-general')
                                </div>
                            </form>
                        </div>
                    </div>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function SubmitForm() {
            ['full_name', 'phone'].forEach(function (value) {
                var a = $("input[name=" + value + "]").val();
                if (!a) {
                   $('#error_'+value).attr("hidden",false)
                }else{
                    $('#error_'+value).attr("hidden",true);
        ...
        function addProduct() {
            ...
            if (!error) {
                ...
                table.innerHTML +=
                    ...
                    '<td style="text-align: left" class="product_' + listProduct.length + '" id="" >' + listProduct[listProduct.length - 1]['nameProduct'] + '</td>' +
                    '<td style="text-align: left" class="product_' + listProduct.length + '" id="" >' + listProduct[listProduct.length - 1]['typeProduct'] + '</td>' +
                    '<td style="text-align: right" class="product_' + listProduct.length + '" id="" >' + listProduct[listProduct.length - 1]['priceProduct'] + '</td>' +
                    '<td style="text-align: right" class="product_' + listProduct.length + '" id="" >' + listProduct[listProduct.length - 1]['qualityProduct'] + '</td>' +
                    '<td style="text-align: right" class="product_' + listProduct.length + '" id="" >' + listProduct[listProduct.length - 1]['sumPrice'] + '</td>' +
                    '<td style="text-align: center" class="product_' + listProduct.length + '" id="" onclick="deleteAction(' + listProduct.length + ')"><i class="fa fa-trash-alt"></i></td>'
